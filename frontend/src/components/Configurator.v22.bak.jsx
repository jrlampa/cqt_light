...
import { Calculator, FolderOpen, LayoutTemplate, Trash2, Save, FileText, Download, Search, Zap, DollarSign, Edit2, Package, ShieldCheck, Info, AlertOctagon } from 'lucide-react';
...
const Configurator = ({ onStateChange }) => {
  ...
  useEffect(() => {
    ...
    // Real-time Engineering Analysis (Cycle 22)
    if (showAssistant && window.api && estruturas.length > 0) {
      // Find a representative pole for stress check (or average/worst case)
  ...
  return (
    <div className="h-full flex gap-4 relative">
      ...
      <div className="w-72 flex flex-col gap-2 overflow-y-auto pr-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-1 rounded-lg transition ml-1 ${showAssistant ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-500 hover:text-blue-600'}`}
              title="Toggle Assistente de Projeto"
            >
        ...
        {/* Designer Assistant Intelligence Panel */}
        {showAssistant && (
        ...
        <button
          onClick={() => {
            // Logic for Smart Spec: Auto-add standard materials based on conductors
            if (condutorMT.label.includes('Spacer')) {
              const kit = { id: Date.now(), codigo_kit: 'K-ZENITH-COMP', descricao_kit: 'KIT COMPACTA AUTO-SUGERIDO', quantidade: 1, tipo: 'auto' };
              setEstruturas(prev => [kit, ...prev]);
            } else {
              const kit = { id: Date.now(), codigo_kit: 'K-ZENITH-CONV', descricao_kit: 'KIT CONVENCIONAL AUTO-SUGERIDO', quantidade: 1, tipo: 'auto' };
              setEstruturas(prev => [kit, ...prev]);
            }
